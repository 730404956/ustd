...
class InfoNCE(nn.Module):
    def __init__(
        ...
        self.prediction_step = p_step
        self.subsample = subsample
        self.batch_size = batch_size
        self.calc_accuracy = cal_acc

        # predict |prediction_step| timesteps into the future
        self.predictor = nn.Linear(gar_hidden, genc_hidden * self.prediction_step, bias=False)
    ...
    def infonce_loss(self, Wc, z, full_z):
        ...
        c the latent representation (B, L, C * self.prediction_step)
        ...
        for k in range(1, self.prediction_step + 1):
            ...
            if self.calc_accuracy:
                predicted = torch.argmax(results, 1).cpu()
                correct = (predicted == true_labels[: (seq_len - k) * self.batch_size]).sum().item()
