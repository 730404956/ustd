...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_mask, g=None):
        x_mask = torch.unsqueeze(x_mask, 1).to(x.dtype)
        x = self.pre(x) * x_mask
...
class SpeakerEncoder(torch.nn.Module):
    ...
    def embed_utterance(self, mel, partial_frames=128, partial_hop=64):
        ...
        if mel_len > partial_frames:
            ...
            mels = torch.stack(tuple(mels), 0)
            stack_mels = mels.reshape((mels.size(0)*mels.size(1),mels.size(2),mel.size(-1)))

            with torch.no_grad():
                partial_embeds = self(stack_mels)
            embed = partial_embeds.reshape((mels.size(0),mels.size(1),-1))
            embed = torch.mean(embed, axis=0)
